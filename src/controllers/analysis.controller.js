...
const getIncomeAnalysisGroupWise = asyncHandler( async ( req, res ) =>
{
    const userId = req.user._id;
    const { month, year } = req.query;
    ...
    try
    {
        ...
        const incomes = await Transaction.aggregate( [
            ...
            {
                $lookup: {
                    from: "transactiongroups", // Collection for categories
                    localField: "group",
                    foreignField: "_id",
                    as: "groupInfo"
                }
            },
            {
                $unwind: {
                    path: '$groupInfo',
                    preserveNullAndEmptyArrays: true, // Keep transactions without a group
                },
            },
            {
                $group: {
                    _id: { $ifNull: [ '$groupInfo.name', 'Ungrouped' ] },
                    totalAmount: { $sum: "$amount" }  // Sum the amounts for each category
                }
            },
            {
...
const getExpenseAnalysisGroupWise = asyncHandler( async ( req, res ) =>
{
    const userId = req.user._id;
    const { month, year } = req.query;
